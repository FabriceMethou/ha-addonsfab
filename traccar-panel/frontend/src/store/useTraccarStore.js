...
function shouldNotify(type, deviceId) {
  try {
    ...
    if (prefs.eventTypes?.[type] === false) return false;
    if (deviceId != null && prefs.devices?.[deviceId] === false) return false;
    return true;
  } catch {
    return true;
  }
...
const useTraccarStore = create((set, get) => ({
  ...
  setPositions: (positionsArray) => {
    ...
    set((s) => ({
      positions: map,
      deviceStillSince: { ...s.deviceStillSince, ...stillSince },
    }));
  },
  ...
  updatePosition: (position) =>
    set((state) => {
      ...
      if (speedKmh < 1) {
        if (!stillSince[position.deviceId])
          stillSince[position.deviceId] = Date.now();
      } else {
  ...
  checkBatteryAlert: (position) => {
    ...
    if (battery <= 20) {
      set((s) => {
        ...
        if (!shouldNotify("lowBattery", deviceId)) return {};
        ...
        return {
          batteryAlerted: next,
          alerts: [newAlert, ...s.alerts].slice(0, MAX_ALERTS),
        };
      });
    } else if (battery > 20) {
      set((s) => {
    ...
    if (type === "geofenceEnter" || type === "geofenceExit") {
      ...
      if (type === "geofenceEnter") {
        ...
        set((s) => {
          ...
          return {
            geofenceArrivals: { ...s.geofenceArrivals, [deviceId]: next },
          };
        });
      }
    } else if (type === "deviceOverspeed") {
      if (!device) return;
      if (!shouldNotify(type, deviceId)) return;
      ...
      if (!device) return;
      if (!shouldNotify("alarm", deviceId)) return;
