...
function getAlertBody(alert) {
  ...
  if (alert.type === 'deviceOverspeed') return `speeding${alert.speedKmh ? ` — ${alert.speedKmh} km/h` : ''}`
  if (alert.type === 'alarm') return ALARM_LABELS[alert.alarmType] ?? `Alarm: ${alert.alarmType}`
  return alert.type
}
