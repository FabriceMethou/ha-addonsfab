...
export default function PlacesList({ mapRef }) {
  ...
  if (geofences.length === 0) {
    return <EmptyState type="noPlaces" />;
  }
      ...
      {sorted.map((gf) => {
        ...
        return (
          <div key={gf.id} className="mb-1">
            ...
            {isSelected && (
              <div className="mx-1 mb-1 rounded-b-xl border border-brand-200 dark:border-brand-700 bg-brand-50 dark:bg-brand-900/20 px-3 py-2">
                {occupants.length === 0 ? (
                  ...
                  occupants.map(({ device, pos }) => {
                    const lastSeen = pos?.fixTime
                      ? formatDistanceToNow(new Date(pos.fixTime), {
                          addSuffix: true,
                        })
                      : "unknown";
                    ...
                    return (
                      <div
                        key={device.id}
                        className="flex items-center justify-between py-1.5 border-b border-brand-100 dark:border-brand-800 last:border-0"
                      >
                        <div>
