...
export default function DeviceTripsView({ device, onBack, mapRef }) {
  ...
  const toggleBubble = useTraccarStore((s) => s.toggleBubbleDevice);
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .catch((err) => {
        if (!cancelled) setTripsError(err.message);
      })
      .finally(() => {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
        <button
          ...
          className="text-brand-500 hover:text-brand-600 dark:text-brand-400 text-sm font-medium flex items-center gap-1"
        >
          ← All
        </button>
        ...
        <div className="ml-auto flex items-center gap-2">
          <button
            ...
            title={
              isPaused ? "Resume location sharing" : "Pause location sharing"
            }
          >
            {isPaused ? "▶ Resume" : "⏸ Pause"}
          </button>
          <button
            onClick={() => toggleBubble(device.id)}
            className={`text-xs font-medium px-2 py-1 rounded-full transition-colors ${
              isBubble
            ...
            title={
              isBubble ? "Show exact location" : "Show approximate location"
            }
          >
            {isBubble ? "● Exact" : "◎ Bubble"}
          </button>
          <PhotoEditor deviceId={device.id} />
        </div>
      </div>
      ...
      {routeError && (
        <p className="text-xs text-red-500 px-3 py-1 flex-shrink-0">
          {routeError}
        </p>
      )}
