      ...
      <div className="relative">
        ...
        <div>
          ...
          {loading ? (
            ...
            <p
              className={`text-2xl font-bold ${colorClass || "text-foreground"}`}
            >
              {value}
            </p>
          )}
...
export default function ReportsPage() {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await reportsAPI.getNetWorth(
        ownerIdParam ? { owner_id: ownerIdParam } : undefined,
      );
  ...
  const { data: tagReportData, isLoading: tagReportLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await reportsAPI.getTagReport(selectedTag, {
        start_date: startDate,
        end_date: endDate,
      });
      return response.data;
        ...
        ? Array.from(
            {
              length:
                ...
                yearByYearData.year_of_first_transaction +
                1,
            },
    ...
    () =>
      incomeExpensesData
        ? [
            {
              name: "Income",
              amount: incomeExpensesData.income,
              fill: "#10b981",
            },
            {
              name: "Expenses",
              amount: incomeExpensesData.expenses,
              fill: "#ef4444",
            },
            {
              name: "Net",
              amount: incomeExpensesData.net,
              fill: incomeExpensesData.net >= 0 ? "#3b82f6" : "#f59e0b",
            },
          ]
        : [],
    [incomeExpensesData],
        ...
        ? Object.values(
            transactionsData.reduce(
              (acc: Record<string, any>, transaction: any) => {
                ...
                // Exclude transfers from cashflow chart (they are not income or expenses)
                ...
                try {
                  ...
                  if (transaction.amount > 0) {
                    acc[dateKey].income = addMoney(
                      acc[dateKey].income,
                      transaction.amount,
                    );
                  } else {
                    acc[dateKey].expenses = addMoney(
                      acc[dateKey].expenses,
                      absMoney(transaction.amount),
                    );
                  }
                } catch (error) {
                  console.warn(
  ...
  const sortedTrendAnalysis = useMemo(() => {
    ...
    return Object.entries(spendingTrendsData.trend_analysis).sort(
      (a: any, b: any) =>
  ...
  const filteredIncomeCategories = useMemo(() => {
    ...
    return yearByYearData.categories.income.filter(
      (item: any) =>
  ...
  const filteredExpenseCategories = useMemo(() => {
    ...
    return yearByYearData.categories.expenses.filter(
      (item: any) =>
        item.name.toLowerCase().includes(yearByYearSearch.toLowerCase()) ||
        item.subcategories?.some((sub: any) =>
          sub.name.toLowerCase().includes(yearByYearSearch.toLowerCase()),
          ...
          <div className="space-y-6 mt-4">
                ...
                {incomeExpensesError && (
                  <div className="p-4 rounded-lg bg-error/10 border border-error/20">
                    <p className="text-error">
                      Failed to load income/expenses data
                    </p>
                  </div>
                )}
                {spendingError && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <KPICard
                ...
                change={
                  netWorthTrendForComparison &&
                  netWorthTrendForComparison.length >= 2
                    ? calculatePercentageChange(
                        netWorthTrendForComparison[
                          netWorthTrendForComparison.length - 1
                        ].net_worth,
                        netWorthTrendForComparison[
                          netWorthTrendForComparison.length - 2
                        ].net_worth,
                    ...
                    <Bar dataKey="amount" radius={[8, 8, 0, 0]}>
                      {incomeExpensesBarData.map((entry, index) => (
                        <Cell
                          ...
                          fill={
                            entry.name === "Income"
                              ? "url(#incomeBarGradient)"
                              : entry.name === "Expenses"
                                ? "url(#expensesBarGradient)"
                                : "url(#netBarGradient)"
                          }
                        />
                      ))}
            ...
            <Card className="p-6 rounded-xl border border-border bg-card/50 backdrop-blur-sm">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-foreground">
                    {transactionsData?.length
                      ? formatCurrency(
                          sumMoney(transactionsData, (t: any) =>
                            absMoney(t.amount),
                          ) / transactionsData.length,
                        )
                      : formatCurrency(0)}
                ...
                <div>
                  ...
                  <p
                    className={`text-xl font-bold ${incomeExpensesData?.net >= 0 ? "text-success" : "text-error"}`}
                  >
                    {incomeExpensesData?.income > 0
          ...
          <div className="space-y-6 mt-4">
            ...
            {trendsLoading ? (
              ...
              <>
                ...
                <Card className="p-6 rounded-xl border border-border bg-card/50 backdrop-blur-sm">
                  ...
                  <ResponsiveContainer width="100%" height={400}>
                    <AreaChart data={spendingTrendsData.trends}>
                      <defs>
                        {spendingTrendsData.all_categories
                          .slice(0, 8)
                ...
                {/* Trend Analysis */}
                {spendingTrendsData.trend_analysis &&
                  Object.keys(spendingTrendsData.trend_analysis).length > 0 && (
                    <Card className="p-6 rounded-xl border border-border bg-card/50 backdrop-blur-sm">
                      <h3 className="text-lg font-semibold text-foreground mb-6">
                        Trend Analysis
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {sortedTrendAnalysis.map(
                          ([cat, analysis]: [string, any]) => {
                  ...
                  <div className="max-h-[400px] overflow-auto">
                    <Card className="overflow-hidden border border-border rounded-lg">
                      <Table>
                        ...
                        <TableBody>
                          {extendedNetWorthData.trend.map(
                            (row: any, idx: number) => {
                              const prevRow =
                                idx > 0
                                  ? extendedNetWorthData.trend[idx - 1]
                                  : null;
                              const change = prevRow
                                ? row.net_worth - prevRow.net_worth
                                : 0;
                              return (
                                <TableRow key={row.date}>
                                  ...
                                  <TableCell className="text-right">
                                    {formatCurrency(row.debts)}
                                  </TableCell>
                                  <TableCell className="text-right font-bold">
                                    {formatCurrency(row.net_worth)}
                                  </TableCell>
                                  <TableCell
                                    className={`text-right font-bold ${change >= 0 ? "text-success" : "text-error"}`}
                                  >
                                    {idx > 0
                                      ? (change >= 0 ? "+" : "") +
          ...
          <div className="space-y-6 mt-4">
            ...
            {yearByYearLoading ? (
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <KPICard
                    ...
                    value={formatCurrency(
                      yearByYearData.summary.total_expenses,
                    )}
                    icon={
                      <ArrowDownCircle size={24} className="text-rose-500" />
                    }
                ...
                {yearByYearData.sankey &&
                  yearByYearData.sankey.links.length > 0 && (
                    <Card className="p-6 rounded-xl border border-border bg-card/50 backdrop-blur-sm">
                      <h3 className="text-lg font-semibold text-foreground mb-4">
                        Money Flow
                      </h3>
                      <div className="h-[400px]">
                        <ResponsiveSankey
                          data={{
                            nodes: yearByYearData.sankey.nodes.map(
                              (node: any) => ({
                                id: node.id,
                                nodeColor:
                                  node.type === "income"
                          ...
                          align="justify"
                          colors={(node: any) => {
                            const nodeData = yearByYearData.sankey.nodes.find(
                              (n: any) => n.id === node.id,
                            );
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {filteredIncomeCategories.map((item: any) => {
                            const isExpanded = expandedIncomeCategories.has(
                              item.name,
                            );
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {filteredExpenseCategories.map((item: any) => {
                            const isExpanded = expandedExpenseCategories.has(
                              item.name,
                            );
                            const hasSubcategories =
                              item.subcategories &&
              ...
              <div className="p-4 rounded-lg bg-primary/10 border border-primary/20">
                <p className="text-foreground-muted">
                  No data available for {yearByYearSelected}
                </p>
              </div>
            )}
          </div>
            ...
            {!selectedTag ? (
              <div className="text-center py-16">
                ...
                <h3 className="text-lg font-semibold text-foreground-muted mb-2">
                  Select a Tag to View Report
                </h3>
                <p className="text-sm text-foreground-muted">
                  Choose a tag from the dropdown above to see detailed analytics
                </p>
              </div>
            ) : tagReportLoading ? (
              <div className="flex justify-center items-center min-h-[40vh]">
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <KPICard
                    ...
                    iconColor={
                      tagReportData.net >= 0 ? "bg-emerald-500" : "bg-rose-500"
                    }
                    colorClass="text-foreground"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <Card className="p-6 rounded-xl border border-border bg-card/50 backdrop-blur-sm">
                    ...
                    {tagReportData.distribution_by_account?.length > 0 ? (
                      <div className="h-[300px]">
                        <ResponsivePie
                          data={tagReportData.distribution_by_account.map(
                            (acc: any) => ({
                              id: acc.account,
                              label: acc.account,
                ...
                {tagReportData.transactions?.length > 0 && (
                  <Card className="p-6 rounded-xl border border-border bg-card/50 backdrop-blur-sm">
                    <h3 className="text-lg font-semibold text-foreground mb-4">
                      Transactions ({tagReportData.transactions.length})
                    </h3>
                    <div className="max-h-[400px] overflow-auto">
                      <Card className="overflow-hidden border border-border rounded-lg">
                        <Table>
                          ...
                          <TableBody>
                            {tagReportData.transactions.map(
                              (transaction: any) => (
              ...
              <div className="p-4 rounded-lg bg-primary/10 border border-primary/20">
                <p className="text-foreground-muted">
                  No data available for the selected tag and date range
                </p>
              </div>
            )}
          </div>
