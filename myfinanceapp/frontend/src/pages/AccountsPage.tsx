...
export default function AccountsPage() {
  ...
  const onAccountSubmit = (formData: AccountFormData) => {
    ...
    if (formData.opening_balance)
      data.opening_balance = parseFloat(formData.opening_balance);
    if (formData.linked_account_id)
      data.linked_account_id = parseInt(formData.linked_account_id);
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        title="Accounts"
        description="Your bank accounts and balances"
            ...
            {summaryData.map((owner: any) => (
              ...
              >
                ...
                <p className="text-2xl font-bold text-primary">
                  {formatCurrency(owner.total_balance)}
                </p>
                <p className="text-sm text-foreground-muted">
                  {owner.account_count} account
                  {owner.account_count !== 1 ? "s" : ""}
                </p>
              </Card>
            ))}
          </div>
        ...
        <TabPanel value={tabValue} index={0}>
          <div className="p-6">
            <div className="flex justify-end gap-2 mb-4">
              ...
              >
                <RefreshCw
                  className={`w-4 h-4 mr-2 ${recalculateBalancesMutation.isPending ? "animate-spin" : ""}`}
                />
                  ...
                  {banksData && banksData.length > 0 ? (
                    banksData.map((bank: any) => {
                      const bankAccountCount =
                        accountsData?.filter((a: any) => a.bank_id === bank.id)
                          .length || 0;
                      return (
                          ...
                          <TableCell className="text-right">
                            <div className="flex gap-1 justify-end">
                              ...
                              <Button
                                ...
                                title={
                                  bankAccountCount > 0
                                    ? "Cannot delete bank with accounts"
                                    : "Delete bank"
                                }
                              >
                                <Trash2 className="w-4 h-4 text-error" />
                ...
                <TableBody>
                  {ownersData && ownersData.length > 0 ? (
                    ownersData.map((owner: any) => {
                      const ownerAccounts =
                        accountsData?.filter(
                          (a: any) => a.owner_id === owner.id,
                        ) || [];
                      // Use summaryData for total balance - it's already converted to display currency
                      const ownerSummary = summaryData?.find(
                        (s: any) => s.owner_id === owner.id,
                      );
                      const ownerTotalBalance =
                        ownerSummary?.total_balance || 0;
                              ...
                              <Button
                                ...
                                title={
                                  ownerAccounts.length > 0
                                    ? "Cannot delete owner with accounts"
                                    : "Delete owner"
                                }
                              >
                                <Trash2 className="w-4 h-4 text-error" />
      ...
      <Dialog open={accountDialog} onOpenChange={setAccountDialog}>
        <DialogContent size="lg">
          <DialogHeader>
            <DialogTitle>
              {editingItem ? "Edit Account" : "Add Account"}
            </DialogTitle>
            <DialogDescription>
              {editingItem
          ...
          <form onSubmit={handleAccountSubmit(onAccountSubmit)}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-4">
              ...
              <FormField
                label="Bank"
                error={accountErrors.bank_id?.message}
                required
              ...
              <FormField
                label="Owner"
                error={accountErrors.owner_id?.message}
                required
                className="sm:col-span-2"
              >
                <Controller
              ...
              <FormField
                label="Account Type"
                error={accountErrors.account_type?.message}
                required
              >
                <Controller
                  name="account_type"
              ...
              <FormField
                label="Currency"
                error={accountErrors.currency?.message}
                required
              >
                <Controller
                  name="currency"
              ...
              <FormField
                ...
                required
                helperText={
                  accountFormValues.account_type !== "investment"
              ...
              <FormField
                ...
                error={accountErrors.opening_balance?.message}
              >
                <Input
                  type="number"
                  step={
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {editingBank
                ...
                : "Add a new bank to organize your accounts."}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {editingOwner
                ...
                : "Add a new owner to assign to accounts."}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog
        ...
        onOpenChange={() => setDeleteBankConfirm(null)}
      >
        <DialogContent size="sm">
          <DialogHeader>
          ...
          <div className="py-4">
            <div className="flex items-center gap-2 p-3 rounded-lg bg-warning/10 border border-warning/20 text-warning">
              ...
              <span className="text-sm">
                Are you sure you want to delete "{deleteBankConfirm?.name}"?
              </span>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              onClick={() => deleteBankMutation.mutate(deleteBankConfirm.id)}
        ...
        <DialogContent size="sm">
          ...
          <div className="py-4">
            <div className="flex items-center gap-2 p-3 rounded-lg bg-warning/10 border border-warning/20 text-warning">
              <AlertTriangle className="w-4 h-4" />
              <span className="text-sm">
                Are you sure you want to delete "{deleteOwnerConfirm?.name}"?
              </span>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              onClick={() => deleteOwnerMutation.mutate(deleteOwnerConfirm.id)}
        ...
        <DialogContent size="xl">
          <DialogHeader>
            <DialogTitle
              ...
              title={`Validate Account Balance: ${validatingAccount?.name ? `${validatingAccount.name} (${validatingAccount.bank_name})` : `${validatingAccount?.bank_name} - ${validatingAccount?.account_type}`}`}
            >
              Validate Account Balance:{" "}
              {validatingAccount?.name
                ? `${validatingAccount.name} (${validatingAccount.bank_name})`
                : `${validatingAccount?.bank_name} - ${validatingAccount?.account_type}`}
            </DialogTitle>
            <DialogDescription>
              Compare the system balance with your actual account balance to
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 rounded-lg bg-info/10 border border-info/20 min-w-0">
                ...
                <p className="text-xl font-bold text-foreground truncate">
                  {validatingAccount &&
                    formatCurrency(
                      validatingAccount.balance,
                      validatingAccount.currency,
                    )}
                </p>
              </div>
              <div className="space-y-1.5 min-w-0">
                <Label htmlFor="validation-actual-balance">
                ...
                <Input
                  ...
                  onBlur={() => {
                    ...
                    const isInvestment =
                      validatingAccount?.account_type === "investment";
                    const formatted = formatBalanceInput(
                      validationForm.actual_balance,
                      isInvestment,
                    );
                    setValidationForm({
                      ...validationForm,
                      actual_balance: formatted,
                    });
                  }}
                  className="w-full"
                />
                    ...
                    <TableBody>
                      {validationHistory.map((val: any) => (
                        <TableRow key={val.id}>
                          ...
                          <TableCell>
                            <Badge
                              variant={val.is_match ? "success" : "error"}
                              size="sm"
                            >
          ...
          <DialogFooter>
            ...
            <Button
              ...
              loading={createValidationMutation.isPending}
              disabled={!validationForm.actual_balance}
            >
              Save Validation
            </Button>
          </DialogFooter>
