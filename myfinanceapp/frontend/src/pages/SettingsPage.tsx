...
export default function SettingsPage() {
  ...
  const { data: categorizerStatus, refetch: refetchCategorizerStatus } =
    useQuery({
  ...
  const updateDisplayCurrencyMutation = useMutation({
    mutationFn: (currency: string) =>
      settingsAPI.update("display_currency", currency),
    onSuccess: () => {
    ...
    onError: (error: any) => {
      setErrorMessage(
        error.response?.data?.detail || "Failed to update display currency",
      );
  ...
  const updateDebugSettingMutation = useMutation({
    ...
    onError: (error: any) => {
      setErrorMessage(
        error.response?.data?.detail || "Failed to update debug setting",
      );
      setTimeout(() => setErrorMessage(""), 3000);
    },
  });
  ...
  const trainCategorizerMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      setSuccessMessage(
        `Model trained successfully with ${data.data.training_samples} transactions!`,
      );
  ...
  const updateCurrencyMutation = useMutation({
    ...
    onError: (error: any) => {
      setErrorMessage(
        error.response?.data?.detail || "Failed to update currency",
      );
      setTimeout(() => setErrorMessage(""), 3000);
    },
  });
    ...
    onError: (error: any) => {
      setErrorMessage(
        error.response?.data?.detail ||
          "Failed to delete currency. It may be in use by accounts.",
      );
      setTimeout(() => setErrorMessage(""), 5000);
    },
  });
  ...
  const handleSubmitCurrency = () => {
    if (
      !currencyForm.code ||
      !currencyForm.name ||
      !currencyForm.exchange_rate_to_eur
    ) {
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 rounded-xl border border-border bg-card/50 backdrop-blur-sm">
        ...
        <div className="border-t border-border pt-4">
          <p className="text-sm text-foreground-muted mb-4">
            Select the currency to use for displaying all amounts in the
            dashboard and reports. All amounts will be automatically converted
            from their original currency to your selected display currency.
          </p>
          <div className="flex items-center gap-4">
            ...
            <div className="pt-7">
              <Button
                onClick={() =>
                  updateDisplayCurrencyMutation.mutate(displayCurrency)
                }
                disabled={
                  updateDisplayCurrencyMutation.isPending ||
                  displayCurrency === (settings as any)?.display_currency
                }
              >
        ...
        <div className="border-t border-border pt-4">
                  ...
                  {Array.isArray(currenciesData) &&
                    currenciesData.map((currency: any) => (
                      <TableRow key={currency.code}>
                        ...
                        <TableCell className="text-right">
                          1 € ={" "}
                          {currency.exchange_rate_to_eur === 1
                            ? "1.00"
                            : reciprocalRate(
                        ...
                        <TableCell className="text-center">
                          <Badge
                            variant={currency.is_active ? "success" : "outline"}
                            size="sm"
                          >
          ...
          <div className="mt-4 p-4 rounded-lg bg-primary/10 border border-primary/20">
            <p className="text-sm text-foreground">
              <strong>Note:</strong> Exchange rates represent how many units of
              EUR equal 1 unit of the currency. For example, if 1 € = 7.5 DKK,
              the exchange rate is 0.134 (1/7.5).
            </p>
          </div>
        </div>
      </Card>
            ...
            <>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="font-semibold text-foreground">
                    {categorizerStatus.total_transactions}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="font-semibold text-foreground">
                    {categorizerStatus.categorized_transactions}
                  </p>
                </div>
              </div>
              ...
              {/* Staleness warning */}
              {categorizerStatus.is_trained &&
                (categorizerStatus.new_since_training ?? 0) > 50 && (
                  <div className="p-4 rounded-lg mb-4 bg-warning/10 border border-warning/30 flex items-start gap-2">
                    <span className="text-warning mt-0.5">⚠</span>
                    <p className="text-sm text-foreground">
                      <span className="font-semibold text-warning">
                        Model may be stale.
                      </span>{" "}
                      {categorizerStatus.new_since_training} new categorized
                      transactions have been added since the last training.
                      Retrain to improve suggestion accuracy.
                    </p>
                  </div>
                )}
      ...
      <Card className="p-6 rounded-xl border border-warning/50 bg-card/50 backdrop-blur-sm">
        <div className="flex items-center gap-2 mb-4">
          ...
          <Badge
            variant="outline"
            className="ml-auto border-warning text-warning"
          >
            Development
          </Badge>
        </div>
        <div className="border-t border-border pt-4 space-y-4">
          ...
          <div className="flex items-center justify-between py-3 px-4 rounded-lg bg-card hover:bg-muted/50 transition-colors">
            ...
            <Switch
              ...
              onChange={(checked) => {
                updateDebugSettingMutation.mutate({
                  key: "debug_mode",
                  value: checked,
                });
              }}
            ...
            <div className="ml-4 pl-4 border-l-2 border-warning/30 space-y-3">
              ...
              <div className="flex items-center justify-between py-2">
                <div>
                  ...
                  <p className="text-xs text-foreground-muted">
                    Automatically recalculate account balances on page load
                  </p>
                </div>
                <Switch
              ...
              <div className="flex items-center justify-between py-2">
                <div>
                  ...
                  <p className="text-xs text-foreground-muted">
                    Display debug information in browser console
                  </p>
                </div>
                <Switch
              ...
              <div className="flex items-center justify-between py-2">
                <div>
                  ...
                  <p className="text-xs text-foreground-muted">
                    Log transaction create/update/delete operations
                  </p>
                </div>
                <Switch
        ...
        <DialogContent>
          ...
          <div className="p-4 rounded-lg bg-primary/10 border border-primary/20 mb-4">
            <p className="text-foreground">
              This will create a backup of your entire financial database
              including all transactions, accounts, and settings.
            </p>
          </div>
          <p className="text-sm text-foreground-muted">
            The backup will be stored securely and can be used to restore your
            data if needed.
          </p>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingCurrency
                ...
                : "Add a new currency to your available options."}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label
                htmlFor="currency-exchange-rate"
                className="text-sm font-medium text-foreground mb-2 block"
              >
              ...
              />
              <p className="text-xs text-foreground-muted mt-1">
                How many EUR equals 1 unit of this currency (e.g., 0.134 means 1
                DKK = 0.134 EUR)
              </p>
            </div>
            <div className="p-4 rounded-lg bg-primary/10 border border-primary/20">
              <p className="text-xs text-foreground">
                <strong>Example:</strong> If 1 € = 7.5 DKK, then 1 DKK = 0.134
                EUR
                <br />
                So the exchange rate should be:{" "}
                {currencyForm.exchange_rate_to_eur !== ""
      ...
      <Dialog
        open={!!deleteCurrencyConfirm}
        onOpenChange={() => setDeleteCurrencyConfirm(null)}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <div className="p-4 rounded-lg bg-warning/10 border border-warning/20 mb-4">
            <p className="text-foreground">
              ...
              <strong>{deleteCurrencyConfirm?.code}</strong> -{" "}
              {deleteCurrencyConfirm?.name}?
            </p>
          </div>
          <p className="text-sm text-foreground-muted">
            This currency will be deactivated and no longer available for new
            accounts. Existing accounts using this currency will not be
            affected.
          </p>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setDeleteCurrencyConfirm(null)}
            >
              Cancel
            </Button>
            <Button
              variant="destructive"
              onClick={() =>
                deleteCurrencyMutation.mutate(deleteCurrencyConfirm.code)
              }
