...
const isDebugEnabled = () => {
  try {
    ...
    if (debugSettings) {
      ...
      return (
        settings.debug_show_logs === true ||
        settings.debug_log_api_calls === true
      );
    }
  } catch (e) {
    // Ignore parse errors
...
api.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    // A more secure alternative is httpOnly cookies, but that requires server-side changes.
    ...
    if (isDebugEnabled()) {
      console.log(
        `[DEBUG API] → ${config.method?.toUpperCase()} ${config.url}`,
        {
...
api.interceptors.response.use(
  (response: AxiosResponse) => {
    ...
    if (isDebugEnabled()) {
      console.log(
        `[DEBUG API] ← ${response.status} ${response.config.method?.toUpperCase()} ${response.config.url}`,
        {
          data: response.data,
        },
      );
  ...
  async (error: AxiosError) => {
    ...
    if (isDebugEnabled()) {
      console.error(
        `[DEBUG API] ← ${error.response?.status || "ERROR"} ${error.config?.method?.toUpperCase()} ${error.config?.url}`,
        {
...
export const currenciesAPI = {
  getAll: (activeOnly: boolean = true) =>
    api.get(`/api/currencies/?active_only=${activeOnly}`),
...
export const accountsAPI = {
  ...
  createBank: (data: Record<string, unknown>) =>
    api.post("/api/accounts/banks/", data),
  updateBank: (id: number, data: Record<string, unknown>) =>
    api.put(`/api/accounts/banks/${id}`, data),
  deleteBank: (id: number) => api.delete(`/api/accounts/banks/${id}`),
  // Owners
  getOwners: () => api.get("/api/accounts/owners/all"),
  createOwner: (data: Record<string, unknown>) =>
    api.post("/api/accounts/owners/", data),
  updateOwner: (id: number, data: Record<string, unknown>) =>
    api.put(`/api/accounts/owners/${id}`, data),
  deleteOwner: (id: number) => api.delete(`/api/accounts/owners/${id}`),
  // Summary
  getSummary: () => api.get("/api/accounts/summary/balances"),
  // Validations
  createValidation: (data: Record<string, unknown>) =>
    api.post("/api/accounts/validations/", data),
  ...
  investigate: (accountId: number) =>
    api.get(`/api/accounts/${accountId}/investigate`),
  searchByBank: (bankName: string) =>
    api.get(`/api/accounts/search/${bankName}`),
...
export const categoriesAPI = {
  ...
  createType: (data: Record<string, unknown>) =>
    api.post("/api/categories/types", data),
  createSubtype: (data: Record<string, unknown>) =>
    api.post("/api/categories/subtypes", data),
  updateType: (id: number, data: Record<string, unknown>) =>
    api.put(`/api/categories/types/${id}`, data),
  deleteType: (id: number) => api.delete(`/api/categories/types/${id}`),
  updateSubtype: (id: number, data: Record<string, unknown>) =>
    api.put(`/api/categories/subtypes/${id}`, data),
...
export const envelopesAPI = {
  getAll: (includeInactive: boolean = false) =>
    api.get(`/api/envelopes/?include_inactive=${includeInactive}`),
  create: (data: Record<string, unknown>) => api.post("/api/envelopes/", data),
  update: (id: number, data: Record<string, unknown>) =>
    api.put(`/api/envelopes/${id}`, data),
  delete: (id: number) => api.delete(`/api/envelopes/${id}`),
  permanentDelete: (id: number) =>
    api.delete(`/api/envelopes/${id}?permanent=true`),
  ...
  addTransaction: (data: Record<string, unknown>) =>
    api.post("/api/envelopes/transactions", data),
};

// Debts API
export const debtsAPI = {
  getAll: (includeInactive: boolean = false) =>
    api.get(`/api/debts/?include_inactive=${includeInactive}`),
  create: (data: Record<string, unknown>) => api.post("/api/debts/", data),
  update: (id: number, data: Record<string, unknown>) =>
    api.put(`/api/debts/${id}`, data),
...
export const investmentsAPI = {
  ...
  getSecurities: (params: Record<string, unknown> = {}) =>
    api.get("/api/investments/securities", { params }),
  createSecurity: (data: Record<string, unknown>) =>
    api.post("/api/investments/securities", data),
  updateSecurity: (id: number, data: Record<string, unknown>) =>
    api.put(`/api/investments/securities/${id}`, data),
  deleteSecurity: (id: number) =>
    api.delete(`/api/investments/securities/${id}`),

  // Holdings
  getHoldings: () => api.get("/api/investments/holdings"),
  createHolding: (data: Record<string, unknown>) =>
    api.post("/api/investments/holdings", data),
...
export const workProfilesAPI = {
  ...
  create: (data: Record<string, unknown>) =>
    api.post("/api/work-profiles/", data),
  update: (ownerId: number, data: Record<string, unknown>) =>
    api.put(`/api/work-profiles/${ownerId}`, data),
  ...
  calculate: (amount: number, ownerId: number, amountCurrency?: string) =>
    api.post("/api/work-profiles/calculate", null, {
      params: {
        amount,
        owner_id: ownerId,
        ...(amountCurrency && { amount_currency: amountCurrency }),
      },
    }),
};
...
export const backupsAPI = {
  ...
  download: (id: string) =>
    api.get(`/api/backups/${id}/download`, { responseType: "blob" }),
  upload: (file: File) => {
  ...
  updateSettings: (data: Record<string, unknown>) =>
    api.put("/api/backups/settings", data),
  cleanup: () => api.post("/api/backups/cleanup"),
  // Cloud backup APIs
  getCloudConfig: () => api.get("/api/backups/cloud/config"),
  updateCloudConfig: (data: Record<string, unknown>) =>
    api.put("/api/backups/cloud/config", data),
  listCloudBackups: () => api.get("/api/backups/cloud/backups"),
  syncToCloud: (backupId: string) =>
    api.post(`/api/backups/cloud/${backupId}/sync`),
};
...
export const budgetsAPI = {
  getAll: (includeInactive: boolean = false) =>
    api.get(`/api/budgets/?include_inactive=${includeInactive}`),
  ...
  getVsActual: (year: number, month: number) =>
    api.get(`/api/budgets/vs-actual/${year}/${month}`),
};
...
export const alertsAPI = {
  ...
  updateEmailSettings: (data: Record<string, unknown>) =>
    api.put("/api/alerts/email", data),
  updateThresholds: (data: Record<string, unknown>) =>
    api.put("/api/alerts/thresholds", data),
  sendTestEmail: (email: string) =>
    api.post("/api/alerts/test-email", { to_email: email }),
  getHistory: (limit: number = 20) =>
    api.get(`/api/alerts/history?limit=${limit}`),
